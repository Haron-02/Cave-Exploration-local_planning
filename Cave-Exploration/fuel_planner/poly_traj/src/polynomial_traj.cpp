...
namespace fast_planner
{
  ...
  {
    const int seg_num = times.size(); // 轨迹段数 = 时间向量的长度

    // Helper to construct the mapping matrix 
    // 定义一个lambda函数，计算阶乘（n! = n×(n-1)×...×1）
    ...
    {
      Dx(k * 6) = positions(k, 0);  // 第k段x轴起点位置
      Dx(k * 6 + 1) = positions(k + 1, 0);  // 第k段x轴终点位置
    ...
    // A*p = d
    // A矩阵：将多项式系数转换为边界条件（位置、速度、加速度）
    // 原理：5阶多项式有6个系数，每个系数对应边界条件的一个线性组合
    Eigen::MatrixXd Ab;
    Eigen::MatrixXd A = Eigen::MatrixXd::Zero(seg_num * 6, seg_num * 6);  // 总矩阵（块对角）
    for (int k = 0; k < seg_num; k++)
    {
      Ab = Eigen::MatrixXd::Zero(6, 6); // 初始化当前段的子矩阵
      for (int i = 0; i < 3; i++) // i=0:位置（0阶导），i=1:速度（1阶导），i=2:加速度（2阶导）
      {
        // 起点（t=0）的i阶导数：只有系数c_i及更高次项有贡献（低次项导数后为0）
        Ab(2 * i, i) = Factorial(i);  // 2*i行：起点i阶导（位置/速度/加速度）
        // 终点（t=times(k)）的i阶导数：需要乘以时间的幂次
        for (int j = i; j < 6; j++) // j从i开始（低次项导数后为0）
