...
namespace fast_planner
{
  ...
  void FastPlannerManager::initPlanModules(ros::NodeHandle &nh)
  {
    ...
    nh.param("manager/planner_dim", pp_.planner_dim_, 2);
    ...
    // preprocess_elements_.reset(new Preprocess(nh, sdf_map_));
    // topo_planner_.reset(new TopoPlanner(nh));
    // topo_planner_->setPreprocessElements(preprocess_elements_);
    // topo_planner_->init();

    // path_execution_.reset(new PathExecution(nh));
    // path_execution_->setPlannerDim(pp_.planner_dim_);
    // if (pp_.planner_dim_ == 2)
    // {
    //   path_execution_->setMap2DManager(preprocess_elements_->map_2d_manager_);
    // }
    // else if (pp_.planner_dim_ == 3)
    // {
    //   path_execution_->setMap3DManager(preprocess_elements_->map_3d_manager_);
    // }
    // else
    // {
    //   std::cerr << pp_.planner_dim_ << std::endl;
    //   ROS_ERROR("planner_dim_ is error!");
    // }

    // 1. 若用“几何路径”：创建A*路径查找器（2个，备用）
    if (use_geometric_path)
    {
      path_finder_.reset(new Astar);
      path_finder_FAEL_.reset(new Astar);
      // path_finder_->setParam(nh);
      // path_finder_->setEnvironment(edt_environment_);
      // path_finder_->init();
      path_finder_->init(nh, edt_environment_);
      path_finder_FAEL_->init(nh, edt_environment_);
    }

    // 2. 若用“动力学路径”：创建动力学A*（比普通A*多考虑速度/加速度）
    if (use_kinodynamic_path)
    {
      kino_path_finder_.reset(new KinodynamicAstar);  // 动力学A*对象
      kino_path_finder_->setParam(nh);  // 读动力学参数（比如最大速度/加速度）
      kino_path_finder_->setEnvironment(edt_environment_);  // 绑定环境，查障碍
      kino_path_finder_->init();  // 完成初始化
    }

    // 3. 若用“B样条优化”：创建10个优化器（多线程备用，同时优化多条轨迹）
    if (use_optimization)
    {
      bspline_optimizers_.resize(10); // 优化器数组扩容到10个
      for (int i = 0; i < 10; ++i)
      {
        bspline_optimizers_[i].reset(new BsplineOptimizer); // 创建每个优化器
        bspline_optimizers_[i]->setParam(nh); // 读优化参数（比如平滑权重、避障权重）
        bspline_optimizers_[i]->setEnvironment(edt_environment_); // 绑定环境，优化时避障
      }
    }

    // 4. 若用“拓扑路径”：创建拓扑PRM（概率路线图，找不同走法的路径）
    if (use_topo_path)
    {
    ...
    // 第四步：创建ROS发布器(发布路径点，方便在RViz里画图看）
    path_points_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/planning/path_points", 1);
  }
  ...
  //  核心规划1：动力学重规划（kinodynamicReplan）
                                           ...
                                           const double &time_lb)
  {
    ...
    for (double ts = 0.0; ts <= duration + 1e-4; ts += dt)
      points.push_back(init_traj.evaluate(ts, 0));            // 每段的端点位置
    boundary_deri.push_back(init_traj.evaluate(0.0, 1));      // 起始点速度
    boundary_deri.push_back(init_traj.evaluate(duration, 1)); // 终止点速度
    boundary_deri.push_back(init_traj.evaluate(0.0, 2));      // 起始点加速度
    boundary_deri.push_back(init_traj.evaluate(duration, 2)); // 终止点加速度
  ...
  void FastPlannerManager::lockSDFMap()
  {
    sdf_map_->sdf_mutex_.lock();
  }

  void FastPlannerManager::unLockSDFMap()
  {
