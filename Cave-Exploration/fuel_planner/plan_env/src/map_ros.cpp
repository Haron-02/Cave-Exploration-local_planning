...
namespace fast_planner
{
  ...
  void MapROS::init()
  {
    ...
    map_free_pub_ = node_.advertise<sensor_msgs::PointCloud2>("/sdf_map/map_free", 10);
    map_inflate_pub_ = node_.advertise<sensor_msgs::PointCloud2>("/sdf_map/occupancy_inflate", 10);
    ...
    local_cloud_pub_ = node_.advertise<sensor_msgs::PointCloud2>("/sdf_map/local_cloud", 10);
  ...
  void MapROS::updateESDFCallback(const ros::TimerEvent & /*event*/)
  {
    ...
    map_->sdf_mutex_.lock();
    auto t1 = ros::Time::now();

    map_->updateESDF3d();

    map_->sdf_mutex_.unlock();
  ...
  void MapROS::proessDepthImage()
  {
    ...
    // k_depth_scaling_factor_:深度图中存储的值与真实深度（单位为m）的比例，通常情况下，深度值以毫米为单位存储在
    // 16位无符号整数中(0~65535)，因此要获得以米为单位的z值，深度图的像素需要除以比例因子1000
    ...
    {
      // 指向深度图片第v行，第depth_filter_margin_列的指针
      ...
      for (int u = depth_filter_margin_; u < cols - depth_filter_margin_; u += skip_pixel_)
      {
        ...
        // 把算出来的世界坐标系下的pt_world放入pointcloud
  ...
  void MapROS::publishMapAll()
  {
    ...
    if (map_->md_->sdf_map_got_)
    {
      for (int x = map_->md_->occupancy_min_i_[0]; x < map_->md_->occupancy_max_i_[0]; ++x)
        for (int y = map_->md_->occupancy_min_i_[1]; y < map_->md_->occupancy_max_i_[1]; ++y)
          for (int z = map_->md_->occupancy_min_i_[2]; z < map_->md_->occupancy_max_i_[2]; ++z)
          {
            ...
            else if(map_->md_->occupancy_buffer_[map_->toAddress(x, y, z)] > map_->mp_->clamp_min_log_ - 1e-3)
            {
              Eigen::Vector3d pos;
              map_->indexToPos(Eigen::Vector3i(x, y, z), pos);
      ...
      map_free_pub_.publish(cloud_msg2);
      ...
      for (int x = map_->md_->occupancy_min_i_[0]; x < map_->md_->occupancy_max_i_[0]; ++x)
        for (int y = map_->md_->occupancy_min_i_[1]; y < map_->md_->occupancy_max_i_[1]; ++y)
          for (int z = map_->md_->occupancy_min_i_[2]; z < map_->md_->occupancy_max_i_[2]; ++z)
          {
            ...
            if (map_->md_->occupancy_buffer_inflate_[map_->toAddress(x, y, z)] > 0.9)
            {
              Eigen::Vector3d pos;
              map_->indexToPos(Eigen::Vector3i(x, y, z), pos);
  ...
  {
    // local只是x和y方向的local，z方向仍保持原来大小
    // 只publish sdf中的occupied cells
    pcl::PointXYZ pt;
    pcl::PointCloud<pcl::PointXYZ> cloud;
    pcl::PointCloud<pcl::PointXYZ> cloud2;
    // 获取并约束localbound的index
