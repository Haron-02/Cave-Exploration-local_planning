...
public:
  bool operator()(Node3Ptr node1, Node3Ptr node2) {
...
public:
  ...
  void init(ros::NodeHandle& nh, const SDFMap::Ptr &sdf_map);
  void reset();
...
private:
  void backtrack(const Node3Ptr& end_node, const Eigen::Vector3d& end);
  ...
  // main data structure
  vector<Node3Ptr> path_node_pool_;
  int use_node_num_, iter_num_;
  std::priority_queue<Node3Ptr, std::vector<Node3Ptr>, NodeComparator1> open_set_;
  std::unordered_map<Eigen::Vector3i, Node3Ptr, matrix_hash<Eigen::Vector3i>> open_set_map_;
