//  构建和维护一个基于 KD 树的拓扑图，用于机器人路径规划
//  拓扑图通过采样自由空间中的点作为节点，连接无碰撞的节点形成边，最终支持最短路径搜索，同时提供可视化功能。
//
...
namespace preprocess
{
    ...
    void IkdTreeTopoGraph::getParamsFromRos()
    {
        ...
        tgp_.sample_size_ = 30;
        if (!ros::param::get(ns + "/sample_size", tgp_.sample_size_))
        {
            ROS_WARN("No sample_size specified. Looking for %s. Default is '30.0'.",
                     (ns + "/sample_size").c_str());
        }
    ...
    void IkdTreeTopoGraph::setCurrentPosition(const Eigen::Vector3d current_pos)
    {
        current_position_.x() = current_pos.x();
        current_position_.y() = current_pos.y();
        current_position_.z() = current_pos.z();
    }
    ...
    bool IkdTreeTopoGraph::findShortestPath(const int &start_v_id, const int &end_v_id, vector<Eigen::Vector3d> &path) const
    {
        utils::Point3DQueue shortest_path;
        std::vector<int> waypoint_ids;
        bool flag = graph_.getShortestPath(start_v_id, end_v_id, waypoint_ids, shortest_path);
        for(auto &waypoint: shortest_path)
            path.push_back(Eigen::Vector3d(waypoint.x(), waypoint.y(), waypoint.z()));
    ...
    void IkdTreeTopoGraph::updateTopoGraphByMapAndViewpoints()
    {
        Point3DSet sample_points; // points meeting sampling requirements
        for (double x = -tgp_.sample_size_/2; x <= tgp_.sample_size_/2; x += tgp_.sample_dist_h_)
        {
            for (double y = -tgp_.sample_size_/2; y <= tgp_.sample_size_/2; y += tgp_.sample_dist_h_)
            {
                for (double z = -tgp_.sample_size_/2; z <= tgp_.sample_size_/2; z += tgp_.sample_dist_v_)
                {
                    Eigen::Vector3d point(current_position_.x() + x, current_position_.y() + y, current_position_.z() + z);
                    if(sdf_map_->getInflateOccupancy(point)!=1 && sdf_map_->getOccupancy(point) == SDFMap::FREE)
                    {
                        // ROS_WARN("sample_points x:%0.2f  y:%0.2f  z:%0.2f", point.x(), point.y(), point.z());
                        Point3D sample_point(point.x(), point.y(), point.z());
                        sample_points.insert(sample_point);
                    }
                }
            }
        }
        // ROS_WARN("sample_points size:%d", sample_points.size());
    ...
    void IkdTreeTopoGraph::growingByMap3dAndSamplePoints(const Point3DSet &sample_points)
    {
        ...
        graph_.getNeighborVertexsIDs(
            current_position_, tgp_.sample_size_ * 1.5 + tgp_.connectable_range_, local_points);
        ...
        suitable_points_.clear();
        suitable_points_ = suitable_points;
    ...
    int IkdTreeTopoGraph::addPointToGraph(const Eigen::Vector3d &current_pos)
    {
        ROS_INFO("add current position to the plan map");
        int current_point_id = -1;

        Point3D current_point(current_pos.x(), current_pos.y(), current_pos.z());

        if (!getPointId(current_point, current_point_id))
        {
            double local_range = 1.5;
            Point3DQueue neighbor_vertexs;
            std::vector<int> neighbor_vertex_ids;
            do
            {
                nearRangeSearch(current_point, local_range,
                                             neighbor_vertexs, neighbor_vertex_ids);
                local_range += 1.5;
            } while (neighbor_vertex_ids.empty());

            current_point_id = addVertex(current_point);

            for (const auto &item : neighbor_vertexs)
            {   
                Eigen::Vector3d neigbor_pos(item.x(), item.y(), item.z());
                if (sdf_map_->isCollisionFreeStraight(current_pos, neigbor_pos))
                {
                    int item_id = -1;
                    if (current_point_id != item_id && getPointId(item, item_id))
                    {
                        addTwoWayEdge(current_point_id, item_id);
                    }
                    else
                    {
