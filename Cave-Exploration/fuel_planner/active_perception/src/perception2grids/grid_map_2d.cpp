...
#include "perception2grids/grid_map_2d.h"
#include <cmath>
...
namespace perception
{
    ...
    void GridMap2D::initialize(double &grid_size, int &grid_x_num, int &grid_y_num, Status2D init_status)
    {
        ...
        grids_ = std::vector<std::vector<Grid2D>>(map_grid_num_[0], std::vector<Grid2D>(map_grid_num_[1], Grid2D(init_status)));
    }
            ...
            for (int J = 0; J < map_grid_num_[1]; ++J)
            {
                if (grids_[I][J].status == Status2D::Occupied)
                {
                    for (int i = -increment; i <= increment; ++i)
                    {
                        ...
                        {
                            if (I + i >= 0 && I + i < map_grid_num_[0] && J + j >= 0 && J + j < map_grid_num_[1])
                            {
                                inflate_grids[I + i][J + j].status = Status2D::Occupied;
                            }
                        }
                    }
                }
                ...
                if (grids_[I][J].status == Status2D::Empty)
                {
                    for (int i = -increment_empty; i <= increment_empty; ++i)
                    {
                        for (int j = -increment_empty; j <= increment_empty; ++j)
                        {
                            if (I + i >= 0 && I + i < map_grid_num_[0] && J + j >= 0 && J + j < map_grid_num_[1])
                            {
                                inflate_grids[I + i][J + j].status = Status2D::Empty;
                            }
                        }
                    }
                }
    ...
    bool GridMap2D::getShortestPath(const Point2D &goal, const Point2D &start, std::vector<Point2D> &path2d) const
    {
        path2d.clear();

        if (isInMapRange2D(start) && isInMapRange2D(goal) &&
            getStatusInMap2D(start) == Status2D::Free && getStatusInMap2D(goal) == Status2D::Free)
        {
            ...
            std::vector<double> dist(map_grid_num_[0] * map_grid_num_[1], INFINITY);       // Vector of distances
            std::vector<double> estimation(map_grid_num_[0] * map_grid_num_[1], INFINITY); // Vector of G + H .
            const int INF = 0x3f3f3f3f;                                                    // integer infinity
            std::vector<int> backpointers(map_grid_num_[0] * map_grid_num_[1], INF);       // Vector of backpointers
            std::vector<status> in_pq(map_grid_num_[0] * map_grid_num_[1], status::INIT);
            ...
            { // Loop until priority queue is empty
                ...
                { // Get all adjacent vertices
                    for (int j = -1; j <= 1; ++j)
                    {
                        if (!(i == 0 && j == 0))
                        {
                            Index2D id_temp = gridIdToIndex2D(u) + Index2D(i, j);
                            if (isInMapRange2D(id_temp))
                            {
                                if (grids_[id_temp.x()][id_temp.y()].status == Status2D::Free)
                                {
                                    // Get vertex label and weight of current adjacent edge of u
                                    v = index2DToGridId(id_temp);
                                    ...
                                    if (dist[v] > dist[u] + grid_size_ * sqrt(i * i + j * j))
                                    {
                                        ...
                                        // Diagonal-heuristic
                                        Point2D diff = (getGridCenter(v) - getGridCenter(goal_grid_id)).cwiseAbs();
                                        double min_dist = minPositionElement(diff);
                                        double max_dist = maxPositionElement(diff);
                                        double heuristic = max_dist + (sqrt(2.0) - 1) * min_dist;
            ...
            else
            {
                ...
                {
                    path2d.push_back(getGridCenter(path[i]));
                }
                path2d.push_back(goal);
                return true;
            }
        }
        else
    ...
    GridMap2D::generateMapMarkers(const std::vector<std::vector<Grid2D>> &grid_map,
