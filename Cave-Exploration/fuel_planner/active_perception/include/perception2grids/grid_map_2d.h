...
namespace perception
{
    ...
    public:
        ...
        GridMap2D(const GridMap2D &other)
        {
            ...
            center_point_ = Point2D(other.center_point_);
            map_grid_num_ = Eigen::Vector2i(other.map_grid_num_);
            map_length_ = Eigen::Vector2d(other.map_length_);
            map_min_bound_ = Eigen::Vector2d(other.map_min_bound_);
            map_max_bound_ = Eigen::Vector2d(other.map_max_bound_);
        ...
        generateMapMarkers(const std::vector<std::vector<Grid2D>> &grid_map, const geometry_msgs::Pose &current_pose);
        ...
        {
            Point2D center;
            center.x() = map_min_bound_[0] + grid_size_ / 2 + grid_size_ * (grid_id / map_grid_num_[1]);
            center.y() = map_min_bound_[1] + grid_size_ / 2 + grid_size_ * (grid_id % map_grid_num_[1]);
            return center;
        }

        inline int index2DToGridId(const Index2D &index) const
        {
            return index.x() * map_grid_num_[1] + index.y();
        };

        inline int index2DToGridId(const int &index_x, const int &index_y) const
        {
            return index_x * map_grid_num_[1] + index_y;
        }

        inline Index2D gridIdToIndex2D(const int &id) const
        {
            Index2D index;
            index.x() = id / map_grid_num_[1];
            index.y() = id % map_grid_num_[1];
            return index;
        };
        ...
        inline bool isNearOccupy(const Index2D &index) const
        {
            
            Index2D neighbor_index = index;
            
            neighbor_index[0] += 1;
            if (getStatusInMap2D(neighbor_index) == Status2D::Occupied)
                return true;
        ...
        inline bool isNearUnknown(const Index2D &index) const
        {
            
            Index2D neighbor_index = index;
            
            neighbor_index[0] += 1;
            if (getStatusInMap2D(neighbor_index) == Status2D::Unknown)
                return true;
        ...
        inline size_t maxElementIndex(const Point2D &pos) const
        {
            return pos[0] >= pos[1] ? 0 : 1;
        }
        ...
        inline Eigen::Vector2d getCenterPoint()
        {
            return this->center_point_;
        }

        inline Eigen::Vector2i getMapGridNum()
        {
            return this->map_grid_num_;
        }

        inline Eigen::Vector2d getMapLength()
        {
            return this->map_length_;
        }

        inline Eigen::Vector2d getMapMinBound()
        {
            return this->map_min_bound_;
        }

        inline Eigen::Vector2d getMapMaxBound()
        {
            return this->map_max_bound_;
        }

        inline std::vector<std::vector<Grid2D>> getMapGrid()
        {
            return this->grids_;
        }
    ...
    private:
        ...
        Eigen::Vector2d map_min_bound_;
        Eigen::Vector2d map_max_bound_;
