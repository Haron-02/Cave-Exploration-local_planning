...
namespace topo_planner
{
    ...
    public:
        ...
        typedef shared_ptr<TopoPlanner> Ptr;
        ...
        ros::Publisher goal_scan_pub_;

        ExplorerPlannerServer planner_action_server_;
        int planner_goal_id_;
        int planner_dim_;

        ros::Timer goal_scan_timer_;
        bool preprocess_inited_;
        bool exploration_finish_;
        bool is_failure_timer_start_;
        bool is_surface_follow_;
        bool is_update_virtual_obstcale_;

        utils::Point3D closest_virtual_obstacle_;

        std::unique_ptr<time_utils::Timer> failure_timer_;

        preprocess::Preprocess::Ptr elements_;

        rapid_cover_planner::RapidCoverPlanner::Ptr planner_;

        TopoPlanner(ros::NodeHandle &nh_private);

        void init();

        void setPreprocessElements(const preprocess::Preprocess::Ptr &elements);

        void setPlannerDim(const int &dim);

        void planGlobalPath(std::vector<control_planner_interface::Path> &path_segments, bool &is_success, bool &is_exploration_finished);

        void planGlobalPathNew(std::vector<control_planner_interface::Path> &path_segments,
        ...
        void goalScanCallback(const ros::TimerEvent &e);
    };
}
