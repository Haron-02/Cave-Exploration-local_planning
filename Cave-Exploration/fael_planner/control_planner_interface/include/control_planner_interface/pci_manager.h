...
namespace interface
{
    ...
    public:
        ...
        PCIManager(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private) : nh_(nh), nh_private_(nh_private),
        ...
        PCIManager(const ros::NodeHandle &nh_private) : nh_private_(nh_private),
                                                        pci_status_(PCIStatus::kReady), force_stop_(false)
        {
            trajectory_vis_pub_ = nh_private_.advertise<visualization_msgs::MarkerArray>(
                "/pci_command_trajectory_vis", 10);
        }
        ...
        virtual bool executePath(const std::vector<control_planner_interface::Path> &path_segments,
                                 bool &success, double &start_time, double &path_length, geometry_msgs::Pose &goal_pose) = 0;
